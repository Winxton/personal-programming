...
def mapper(record):
    ...
    for i in range(0, 11): 
        if (counts[straightrunfaces[i]] 
            and counts[straightrunfaces[i+1]] 
            and counts[straightrunfaces[i+2]] 
            and counts[straightrunfaces[i+3]]):
            is_straight4 = True
            if (i+4 < len(straightrunfaces) and counts[straightrunfaces[i+4]]): 
                is_straight = True 
                break
